...
import static pascal.taie.analysis.dataflow.inter.InterConstantPropagation.valueMap;
...
public class ConstantPropagation extends
    ...
    public static Value evaluate(Exp exp, CPFact in) {
        ...
        if(exp instanceof IntLiteral intLiteral) {
            ...
            if(in.get(var).isConstant()) return Value.makeConstant(in.get(var).getConstant());
            return in.get(var);
        }
            ...
            for(Obj obj : pta.getPointsToSet(access.getBase())){
                value = meetValue(value, valueMap.getOrDefault(new Pair<>(obj, access.getFieldRef()), Value.getUndef()));
            }
            return value;
        }
        else if(exp instanceof StaticFieldAccess access){
            return valueMap.getOrDefault(new Pair<>(access.getFieldRef().getDeclaringClass(), access.getFieldRef()), Value.getUndef());
        }
            ...
            if(index.isConstant()){
                for(Obj obj : pta.getPointsToSet(access.getBase())){
                    value = meetValue(value, valueMap.getOrDefault(new Pair<>(obj, index), Value.getUndef()));
                    value = meetValue(value, valueMap.getOrDefault(new Pair<>(obj, Value.getNAC()), Value.getUndef()));
                }
            }
            else if(index.isNAC()){
                for(Obj obj : pta.getPointsToSet(access.getBase())){
                    for(Map.Entry<Pair<?, ?>, Value> entry : valueMap.entrySet()){
                        Pair<?, ?> acesspair = entry.getKey();
                        if(acesspair.first().equals(obj) && acesspair.second() instanceof Value){
